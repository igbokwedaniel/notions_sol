...
class proth_prime_checker
{
    ...
    public:
        ...
        proth_type run();
        long long powa(long long number, long exponent){return std::pow(number, exponent);}
        long long powa_modulo(long long number, long exponent, long long modulus){return powa(number, exponent) % modulus;}
